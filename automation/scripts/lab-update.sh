#!/usr/bin/env bash
# =============================================================================
# lab-update.sh — Lab update and commit helper
# Usage: bash automation/scripts/lab-update.sh
# =============================================================================

REPO_DIR="$(git -C "$(dirname "$0")" rev-parse --show-toplevel)"
DATE=$(date +%Y-%m-%d)
DAILY_LOG="$REPO_DIR/build-log/daily-log.md"
CHANGE_LOG="$REPO_DIR/build-log/change-log.md"

GREEN='\033[0;32m'
CYAN='\033[0;36m'
YELLOW='\033[1;33m'
BOLD='\033[1m'
RESET='\033[0m'

divider() { echo -e "${CYAN}──────────────────────────────────────────${RESET}"; }
header()  { echo -e "\n${BOLD}${GREEN}$1${RESET}"; divider; }

ask() {
    local varname="$1"
    local msg="$2"
    echo -e "${YELLOW}${msg}${RESET}"
    read -r inputval
    printf -v "$varname" '%s' "$inputval"
}

confirm() {
    echo -e "${YELLOW}$1 [y/N]:${RESET}"
    read -r ans
    [[ "$ans" =~ ^[Yy]$ ]]
}

# --- Daily log ---------------------------------------------------------------
daily_log_entry() {
    header "📓 Daily Log — $DATE"

    ask phase      "Phase (e.g. Phase 0 — Foundation):"
    ask time_spent "Time spent (e.g. ~2h):"
    ask worked_on  "What did you work on?"
    ask changed    "What did you change? (Enter to skip)"
    ask worked     "What worked?"
    ask failed     "What failed? (Enter to skip)"
    ask learned    "What did you learn?"
    ask next_step  "What is the next step?"

    cat >> "$DAILY_LOG" <<EOF

## $DATE

**Phase:** ${phase:-—}
**Time spent:** ${time_spent:-—}

### What I worked on
${worked_on:-—}

### What I changed
${changed:-n/a}

### What worked
${worked:-—}

### What failed
${failed:-n/a}

### What I learned
${learned:-—}

### Next step
${next_step:-—}
EOF

    echo -e "${GREEN}✔ Daily log updated${RESET}"
}

# --- Change log --------------------------------------------------------------
change_log_entry() {
    header "📋 Change Log — $DATE"

    ask cl_added   "Added (Enter to skip):"
    ask cl_changed "Changed (Enter to skip):"
    ask cl_fixed   "Fixed (Enter to skip):"
    ask cl_broke   "Broke (Enter to skip):"

    cat >> "$CHANGE_LOG" <<EOF

## $DATE

### Added
- ${cl_added:-n/a}

### Changed
- ${cl_changed:-n/a}

### Fixed
- ${cl_fixed:-n/a}

### Broke
- ${cl_broke:-n/a}
EOF

    echo -e "${GREEN}✔ Change log updated${RESET}"
}

# --- Phase task tick-off -----------------------------------------------------
tick_phase_task() {
    header "✅ Tick Off a Phase Task"

    echo "Available phase files:"
    local i=1
    local files=()
    while IFS= read -r f; do
        echo "  $i) $(basename "$f")"
        files+=("$f")
        i=$((i + 1))
    done < <(find "$REPO_DIR/phases" -name "*.md" | sort)

    echo ""
    ask phase_num "Pick a phase file number:"
    local chosen="${files[$((phase_num - 1))]}"
    echo -e "File: ${CYAN}$(basename "$chosen")${RESET}\n"

    local j=1
    local linenums=()
    local tasktexts=()

    while IFS= read -r rawline; do
        local linenum
        linenum=$(echo "$rawline" | cut -d: -f1)
        local tasktext
        tasktext=$(echo "$rawline" | cut -d: -f2-)
        echo "  $j) $tasktext"
        linenums+=("$linenum")
        tasktexts+=("$tasktext")
        j=$((j + 1))
    done < <(grep -n '^\- \[ \]' "$chosen")

    if [ ${#linenums[@]} -eq 0 ]; then
        echo -e "${YELLOW}No open tasks found in this file.${RESET}"
        return
    fi

    echo ""
    ask task_num "Pick a task number to mark done:"
    local idx=$((task_num - 1))
    local linenum="${linenums[$idx]}"
    local tasktext="${tasktexts[$idx]}"

    sed -i "${linenum}s/- \[ \]/- [x]/" "$chosen"
    echo -e "${GREEN}✔ Marked done:${tasktext}${RESET}"
}

# --- Commit and push ---------------------------------------------------------
commit_and_push() {
    header "🚀 Commit and Push"
    cd "$REPO_DIR"

    git add -A

    if git diff --cached --quiet; then
        echo -e "${YELLOW}Nothing to commit.${RESET}"
        return
    fi

    echo "Files changed:"
    git --no-pager diff --cached --name-only
    echo ""

    ask commit_msg "Commit message (Enter for default):"
    if [ -z "$commit_msg" ]; then
        commit_msg="Lab update: $DATE"
    fi

    git commit -m "$commit_msg

Co-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>"

    echo -e "${CYAN}Pushing...${RESET}"
    if git push; then
        echo -e "${GREEN}✔ Pushed successfully${RESET}"
    else
        echo -e "${YELLOW}⚠ Push failed — run 'git push' manually${RESET}"
    fi
}

# --- Menu --------------------------------------------------------------------
show_menu() {
    header "🧪 Lab Update Script"
    echo "  1) Daily log entry"
    echo "  2) Change log entry"
    echo "  3) Tick off a phase task"
    echo "  4) All of the above"
    echo "  5) Quick commit + push (no log changes)"
    echo "  q) Quit"
    divider
}

# --- Main --------------------------------------------------------------------
main() {
    show_menu
    echo -e "${YELLOW}Choose an option:${RESET}"
    read -r choice

    case "$choice" in
        1) daily_log_entry;  commit_and_push ;;
        2) change_log_entry; commit_and_push ;;
        3) tick_phase_task;  commit_and_push ;;
        4)
            daily_log_entry
            change_log_entry
            if confirm "Tick off a phase task too?"; then
                tick_phase_task
            fi
            commit_and_push
            ;;
        5) commit_and_push ;;
        q|Q) echo "Bye!"; exit 0 ;;
        *) echo -e "${YELLOW}Invalid option: '$choice'${RESET}"; exit 1 ;;
    esac
}

main
